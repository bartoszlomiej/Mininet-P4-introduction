...
control cIngress(inout headers hdr, inout metadata meta, inout standard_metadata_t stdmeta){
    ...
    counter(64, CounterType.packets) count_parsed;
    direct_counter(CounterType.packets) count_dropped;
    
    action drop(){
	count_dropped.count(); //counts dropped packets on all ports
	...
	count_parsed.count((bit<32>) stdmeta.ingress_port); //counts
    }
...
control cEgress(inout headers hdr,  inout metadata meta, inout standard_metadata_t stdmeta){
    ...
    counter(64, CounterType.packets) count_sent;

    action updateCounter(){
	count_sent.count((bit<32>) stdmeta.egress_spec);
    }
